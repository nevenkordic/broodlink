...
```
  ...
  │    Dolt      │  │  Postgres   │  │    NATS     │
  │  :3307       │  │  :5432      │  │  :4222      │
  │ (versioned)  │  │ (hot paths) │  │ (token auth)│
  └──────────────┘  └──────┬──────┘  └──────┬──────┘
                           │                │
                    ┌──────▼──────┐  ┌──────▼──────┐
  ...
  │ 127.0.0.1    │  │  :3311       │  │ 127.0.0.1    │
  │  :3312       │  │              │  │  :3313       │
  │(256-bit sess)│  └──────────────┘  │(cmd allowlist)│
  └──────┬───────┘                    └──────────────┘
         │
  ┌──────▼───────┐
  │  Hugo Site   │
  ...
  All HTTP services bind 127.0.0.1 only — use a reverse proxy for external access.
  CORS restricted to explicit origin allowlist. TLS enforced in non-dev profiles.
```
