...
const _ACTION_RECENTS_KEY = 'broodlink-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'broodlink-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Broodlink';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Broodlink' : shortModel(resolvedModel);
