...
let _sortMode = Storage.get('broodlink-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Broodlink Chat';
...
const FOLDER_STATE_KEY = 'broodlink-folder-state';
const FOLDER_ORDER_KEY = 'broodlink-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Broodlink kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Broodlink Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('broodlink-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('broodlink-session-sort', mode);
  else Storage.remove('broodlink-session-sort');
  renderSessionList();
}
