...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'broodlink-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('broodlink-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('broodlink-compare-synth-' + state._compareMode);
