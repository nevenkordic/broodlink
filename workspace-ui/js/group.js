...
const GROUP_STATE_KEY = 'broodlink-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('broodlink-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('broodlink-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
