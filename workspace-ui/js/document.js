...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__broodlinkActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'broodlink-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'broodlink-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__broodlinkActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('broodlink-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('broodlink-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'broodlink-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('broodlink-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('broodlink-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('broodlink-suggestions-' + docId);
      if (!raw) return;
