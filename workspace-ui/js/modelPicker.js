...
function _initModelPickerDropdown() {
  ...
  function _populate(filter) {
    ...
    // Load favorites
    const favs = (function() { try { return JSON.parse(localStorage.getItem('broodlink-model-favorites') || '[]'); } catch { return []; } })();
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('broodlink:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('broodlink:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `broodlink-model-favorites[0]`
